...
echo "ssh -o StrictHostKeyChecking=no -t -i ~/${fullpath##*/} ec2-user@$curr_ip 'source ~/.bash_profile && touch left_cluster.note && sh update_snapshot.sh $3 $4 && touch snapshot_saved.note && pcluster delete $1 && touch cluster_deleted.note'" >> shutdown_cluster.sh
